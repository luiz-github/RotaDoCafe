...
export default function CreateEventScreen({ navigation }) {
  ...
  const useCurrentLocation = async () => {
    try {
      ...
      const locationInfo = await extractCityFromReverseGeocode(latitude, longitude);

      if (locationInfo) {
        if (!isCityAllowed(locationInfo.city)) {
          showError(`Você está fora das cidades do Vale do Café (${locationInfo.city || "local desconhecido"})`);
          setIsSearching(false);
          return;
        }
  ...
  const confirmFullScreenSelection = async () => {
    try {
      ...
      if (locationInfo) {
        if (!isCityAllowed(locationInfo.city)) {
          showError(`Local fora das cidades do Vale do Café (${locationInfo.city || "desconhecido"})`);
          ...
          <View className="bg-gray-900 px-4 py-3 flex-row items-center justify-between">
            ...
            >
              {isReverseGeocoding ? (
                ...
                <Text className="text-amber-400 font-bold text-lg">Confirmar</Text>
              )}
            </TouchableOpacity>
          </View>
          ...
          <View className="bg-gray-900 px-4 py-4 border-t border-gray-700">
            {fullScreenAddress ? (
              <>
                ...
                {fullScreenCity && (
                  <Text className={`text-xs mb-1 ${isCityAllowed(fullScreenCity) ? 'text-green-400' : 'text-red-400'}`}>
                    {isCityAllowed(fullScreenCity) ? '✓' : '✕'} Cidade: {fullScreenCity}
                  </Text>
                )}
              </>
            ) : (
            ...
            <Text className="text-gray-500 text-xs">
              Lat: {fullScreenCoords.latitude.toFixed(5)}, Lng: {fullScreenCoords.longitude.toFixed(5)}
            </Text>
          </View>
        </SafeAreaView>
      </Modal>
